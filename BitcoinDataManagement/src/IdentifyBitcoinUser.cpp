...
void IdentifyBitcoinUsers(){
	...
	useridFileName.open(string(MAC_PATH+"userid.txt").c_str(),ofstream::out);
...
//	tr(AT,it){
//		cout << "TXID = " << it->txID << " address = " << it->addrID << " ";
//		if(it->txIn){
//			cout << "TXIN started from pose = " << it->txIn_startPos << endl;
//		}else{
//			cout << "TXOut started from pose = " << it->txOut_startPos << endl;
//		}
//	}
...
//								cout << "Address got Current User ID = " << current_user_id << endl;
							...
							tmpTX.r_flag = true;  //make it visited
						}
					}
				}//endif
				addrStack.clear();
		...
		// for output addresses in the output txs after consecutive input txs
			...
			if(GotUserID[tmpTX.addrID] == false){  					// if user id for this output address not yet determined
				...
				current_user_id++;									//increase the user_id
//				cout << "Output addrID = " << tmpTX.addrID << " Candidate for new userID. Id incremented to " << current_user_id << endl;
				AT[r].r_flag = true;								//row visited
				addrStack.push_back(tmpTX.addrID);					//push the address of the tx
				userID[tmpTX.addrID] = current_user_id;
				GotUserID[tmpTX.addrID] = true;
...
//							cout << "Output Address Matched. Count = " << cnt << "times." << endl;
							for(long int k = txInStartPos+1; k <= s;k++){
//								cout << "Giving all address ids userid(output) " << current_user_id << " from txin pos=" << txInStartPos << endl;
								tmpTX = AT[k];
								//push addresses if already not exists
								if(find(addrStack.begin(),addrStack.end(),tmpTX.addrID) != addrStack.end()){
...
//				cout << "This output address already got user id." << endl;
			...
			addrStack.clear(); //clear the address stack
		}
	}

	cout << "hi" << endl;
	for(unsigned long int i = 0; i <= userID.size();i++){
		useridFileName << i << "\t" << userID[i] << endl;
	}
}
